...
export const VerificationQueue: React.FC<VerificationQueueProps> = ({
  ...
  const [filter, setFilter] = useState<'all' | 'audit_pending' | 'disputed' | 'forwarded_to_revenue' | 'forwarded_to_tehsildar'>('all');
      ...
      <div className="p-3.5 border-b border-[#d2d2c8] bg-[#efefea] space-y-2.5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] bg-blue-100 border border-blue-300 text-blue-800 font-bold px-1.5 py-0.5 rounded">
              {officialRole}
            </span>
            <span className="text-[10px] bg-amber-100 border border-amber-300 text-amber-800 font-bold px-1.5 py-0.5 rounded">
              {currentTehsil}
            </span>
            {onCloseQueue && (
              <button
        ...
        <div className="flex items-center gap-1 text-[11px] overflow-x-auto pb-1">
          ...
          <button
            ...
            className={`px-2 py-1 rounded-lg transition font-semibold cursor-pointer whitespace-nowrap ${
              filter === 'all'
          ...
          <button
            ...
            className={`px-2 py-1 rounded-lg transition flex items-center gap-1 font-semibold cursor-pointer whitespace-nowrap ${
        ...
        {isLoading ? (
          ...
          <div className="py-16 px-4 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto">
              All structures for <span className="text-blue-700 font-bold">{officialRole}</span> in <span className="text-amber-700 font-bold">{currentTehsil}</span> have been actioned.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  ...
                  {props.status === 'forwarded_to_revenue' ? (
                    <span className="text-[10px] bg-blue-100 text-blue-800 font-bold px-2 py-0.5 rounded-md border border-blue-300">
                      Fwd: Revenue
                    </span>
                  ) : props.status === 'forwarded_to_tehsildar' ? (
                    <span className="text-[10px] bg-cyan-100 text-cyan-800 font-bold px-2 py-0.5 rounded-md border border-cyan-300">
                      Fwd: Tehsildar
                    </span>
                    ...
                    {officialRole === 'Lekhpal' && (
                      <div className="grid grid-cols-2 gap-1.5">
                        <button
                          ...
                          className="flex items-center justify-center gap-1 bg-[#29cc39] hover:bg-[#22a229] text-white text-[11px] font-bold py-1.5 rounded-lg transition shadow-sm disabled:opacity-50 cursor-pointer"
                        >
                        ...
                        <button
                          disabled={isBusy}
                          onClick={() => handleAction(props.structure_id, 'forward_to_revenue')}
                          title="Forward to Revenue Inspector"
                          className="flex items-center justify-center gap-1 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold py-1.5 rounded-lg transition shadow-sm disabled:opacity-50 cursor-pointer"
                        >
                          <span>Fwd to Revenue</span>
                        </button>
                      </div>
                    )}

                    {/* Revenue Inspector Actions */}
                    {officialRole === 'Revenue Inspector' && (
                      <button
                        disabled={isBusy}
                        onClick={() => handleAction(props.structure_id, 'forward_to_tehsildar')}
                        title="Forward to Tehsildar"
                        className="w-full flex items-center justify-center gap-1 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold py-1.5 rounded-lg transition shadow-sm disabled:opacity-50 cursor-pointer"
                      >
                        <span>Forward to Tehsildar for Final Approval</span>
                      </button>
                    )}

                    {/* Tehsildar Actions */}
                    {officialRole === 'Tehsildar' && (
                      <button
                        disabled={isBusy}
                        onClick={() => handleAction(props.structure_id, 'tehsildar_commit')}
                        title="Final Commit to PostGIS Database"
                        className="w-full flex items-center justify-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold py-1.5 rounded-lg transition shadow-sm disabled:opacity-50 cursor-pointer"
                      >
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        <span>Commit Final Approval to PostGIS DB</span>
                      </button>
                    )}

                    {/* Lock Disputed Button (Available to all roles) */}
                    <button
                      disabled={isBusy}
                      onClick={() => handleAction(props.structure_id, 'lock_disputed')}
                      title="Lock as Disputed"
                      className="w-full flex items-center justify-center gap-1 bg-red-600 hover:bg-red-700 text-white text-[11px] font-bold py-1 rounded-lg transition shadow-sm disabled:opacity-50 cursor-pointer"
                    >
